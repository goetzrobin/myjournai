...
function FinalConvo() {
  ...
  const { data: sessionLog, isSuccess: isSuccessSessionLog } = useLatestSessionLogBySlugQuery({ slug: 'onboarding-v0', userId });
  ...
  const isShowingChat = userId && conversationInitialized && isSuccessSessionLog;
  ...
  useEffect(() => {
    if (completeOnboardingMutation.isIdle && !conversationInitialized.current && isSuccessSessionLog && !sessionInProgress) {
      ...
      setTimeout(() => completeOnboardingMutation.mutate(), 50);
    }
    ...
    {!isShowingChat ? null :
      <Chat isShowingUserInput={isShowingUserInput} isMessageSuccess={sessionLogMessagesQuery.isSuccess} messages={sessionLogMessagesQuery.data ?? []} userId={userId}>
